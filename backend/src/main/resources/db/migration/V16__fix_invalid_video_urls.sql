-- V16: Fix invalid and embedding-disabled YouTube video URLs
-- Root cause: Old seed video IDs (ruI2tdQzprg, DywRyvzWoYY, 5MFQH5IHOFk, listType=user_uploads) are either deleted, private, or have third-party embedding disabled by owner on YouTube → "Video không có sẵn / Video này không hoạt động"
...
-- 2. Replace broken video ID 'ruI2tdQzprg' (used in Lesson 33, 9, 21, 27, 30) with working Grammar/Conditionals video ID 'kJQP7kiw5Fk'
UPDATE lessons
SET video_url = 'https://www.youtube.com/watch?v=kJQP7kiw5Fk',
    lesson_type = CASE WHEN lesson_type = 'TEXT' THEN 'MIXED' ELSE lesson_type END
WHERE video_url LIKE '%ruI2tdQzprg%';
