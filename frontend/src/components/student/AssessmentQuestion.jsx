...
export default function AssessmentQuestion({ question, answer, disabled, onAnswer, saving }) {
  ...
  return (
    ...
    >
      <div style={{ flex: 1, overflowY: "auto", minHeight: 0, paddingRight: "0.35rem" }}>
        <div className="assessment-question-meta" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.5rem" }}>
          <span style={{ fontSize: "0.75rem", fontWeight: "700", textTransform: "uppercase", background: "#e0f2fe", color: "#0369a1", padding: "0.2rem 0.55rem", borderRadius: "6px" }}>
            {qType.replaceAll("_", " ")}
          </span>
          <strong style={{ fontSize: "0.85rem", color: "#64748b" }}>{points} Điểm</strong>
        </div>

        <h3 style={{ fontSize: "1.1rem", fontWeight: "700", color: "#0f172a", margin: "0.5rem 0 1rem 0", lineHeight: "1.45" }}>
          {qText}
        </h3>

        {choiceType && options.length > 0 && (
          <div className="assessment-options" style={{ display: "flex", flexDirection: "column", gap: "0.65rem" }}>
            {options.map((option) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span style={{ fontSize: "0.95rem", color: checked ? "#0f766e" : "#1e293b" }}>{optionLabel}</span>
                </div>
              );
            })}
          </div>
      ...
      {(saving || disabled) && (
        <div style={{ paddingTop: "0.4rem", flexShrink: 0 }}>
          {saving && <small className="assessment-saving" style={{ color: "#0d9488", display: "block" }}>Đang tự động lưu...</small>}
